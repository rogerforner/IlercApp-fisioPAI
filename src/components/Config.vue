<template>
  <v-layout row justify-center>
    <!-- Contingut -->
    <v-dialog v-model="dialogs.isConfigVisible" fullscreen hide-overlay transition="dialog-bottom-transition">
      <v-card>
        <!-- Barra superior -->
        <v-toolbar dark color="primary">
          <v-btn icon dark @click="dialogs.isConfigVisible = false">
            <v-icon>fa-times-circle</v-icon>
          </v-btn>
          <v-toolbar-title>Configuració</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-toolbar-items>
...
<script>
  export default {
    name: "Config",
    created () {
      this.$eventBus.$on("navConfig:change", () => {
        this.dialogs.isConfigVisible = true;
      })
    },
    data: () => ({
      dialogs: {
